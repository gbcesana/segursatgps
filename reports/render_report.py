...
class RenderReport:
    ...
    def __get_unit(self,units,unitid):
        unit = units.get(id=int(unitid))
        if unit.is_child:
            try:
    ...
    def render_location_report(self,request):
        data = request.data
        serializer = report_serializers.ReportSerializer(data=data)
        if serializer.is_valid():
            try:
            ...
            try:
                ...
                return Response(error,status=status.HTTP_400_BAD_REQUEST)
            return Response(
                report.generate_location_history(
                    unit,
                    initial_timestamp,
                    final_timestamp,
                ),
    ...
    def render_long_speed_report(self,request):
        data = request.data
        serializer = report_serializers.SpeedReportSerializer(data=data)
        if serializer.is_valid():
            try:
            ...
            if int(data['unitid']) == 0:
                ...
                try:
                    ...
                    return Response(error,status=status.HTTP_400_BAD_REQUEST)
                return Response(
                    report.generate_long_speed_report(
                        unit,
                        initial_timestamp,
                        final_timestamp,
                        data['speed_limit']
